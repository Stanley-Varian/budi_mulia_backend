...
export const getJadwal = async (req, res) => {
  try {
    ...
    if (!kelas)
      return res
        .status(400)
        .json({ success: false, message: "Data kelas siswa tidak ditemukan." });
  ...
  try {
    ...
    if (kelasDefault) {
      ...
      for (const m of mapelDariMateri) {
        if (!semuaMapel.has(m.mapel)) {
          semuaMapel.set(m.mapel, {
            id: null,
            mapel: m.mapel,
            guru: m.guru,
    ...
    const result = Array.from(semuaMapel.values()).sort((a, b) =>
...
export const joinKelas = async (req, res) => {
  try {
    ...
    if (!kodeKelas || !kodeKelas.trim()) {
      return res
        .status(400)
        .json({ success: false, message: "Kode kelas tidak boleh kosong." });
    }
    ...
    if (!kelas) {
      return res
        ...
        .json({
          success: false,
          message:
            "Kode kelas tidak ditemukan. Pastikan kode yang kamu masukkan benar.",
        });
    }

    const sudahGabung = kelas.anggota.some(
    ...
    if (sudahGabung) {
      return res
        ...
        .json({
          success: false,
          message: `Kamu sudah terdaftar di kelas ${kelas.nama}.`,
        });
    }
...
export const getKelasSiswa = async (req, res) => {
  try {
    const siswaId = req.user._id;
    const kelasList = await Kelas.find({ anggota: siswaId }).sort({
      createdAt: -1,
    });
...
export const getEkskul = async (req, res) => {
  try {
    const ekskulList = await Ekskul.find({ aktif: true }).sort({
      hari: 1,
      jam: 1,
    });
    res.json({
